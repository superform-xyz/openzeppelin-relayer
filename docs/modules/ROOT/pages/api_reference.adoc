...
|Submit a transaction to the Stellar network. Supports payment and InvokeHostFunction operations.
...
|Execute Soroban smart contract operations including contract calls, deployments, and WASM uploads
|===
...
**InvokeHostFunction Operation Examples:**
...
2. **Deploy Contract (Create Contract):**
[source,bash]
----
curl --location --request POST 'http://localhost:8080/api/v1/relayers/<stellar_relayer_id>/transactions' \
--header 'Authorization: Bearer <your_api_key>' \
  ...
  "operations": [
    {
      "op": "invoke_host_function",
      "upload_wasm": {
        "wasm": "AGFzbQEAAAABBgFgAX8BfwMCAQAFAwEAAQcPAgVoZWxsbwAACG1lbW9yeTIDCgQAAAAL"
      }
    }
  ]
}'
...
When using the `invoke_host_function` operation with contract calls, arguments must be provided as ScVal values in JSON format. The relayer uses the stellar-xdr JSON serialization format.
...
The `invoke_host_function` operation supports different authorization modes for Soroban operations:

===== Default Authorization
If no `auth` field is provided, the relayer automatically generates appropriate authorization entries based on the operation type.
...
===== XDR Authorization
For advanced use cases, provide complete authorization entries as base64-encoded XDR:
[source,json]
----
{
  "op": "invoke_host_function",
