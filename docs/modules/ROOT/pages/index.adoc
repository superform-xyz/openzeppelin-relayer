...
NOTE: For EVM transaction signing, ensure your Google Cloud KMS key is created with protection level HSM, Asymmetric sign purpose and the algorithm "Elliptic Curve secp256k1 - SHA256 Digest" which provides secp256k1 compatibility required for Ethereum transactions.
...
Some tags have special meaning and affect the relayer's behavior:
...
|`optimism`
|Identifies Optimism-based networks and Layer 2s using the OP Stack (e.g., Optimism, Base, World Chain).
